	...
	size_t count, loff_t pos, loff_t *ppos)
{
	...
	do {
		...
		if (bytes > count)
			bytes = count;
		//printk("**xip.c** %s()->count=%u,pmfs_get_xip_mem()->__pmfs_get_block()->pmfs_find_and_alloc_blocks().\n",__func__,count);
...
ssize_t pmfs_xip_file_write(struct file *filp, const char __user *buf,
	size_t len, loff_t *ppos)
{
	printk("**xip.c** pmfs_xip_file_write() start.\n");
	...
	printk("**xip.c** %s()->pmfs_find_data_block() once.\n",__func__);
	...
	printk("**xip.c** %s()->pmfs_find_data_block() twice.\n",__func__);
	...
	printk("**xip.c** %s()-> __pmfs_xip_file_write() .\n",__func__);
...
out:
	...
	printk("**xip.c** pmfs_xip_file_write() end.\n");
	return ret;
}
	...
	if (!block) {
		...
		if (trans) {
			...
			pmfs_add_logentry(sb, trans, pi, MAX_DATA_PER_LENTRY,
				LE_DATA);
			printk("**xip.c** %s()->pmfs_alloc_blocks().\n",__func__);
			err = pmfs_alloc_blocks(trans, inode, iblock, 1, true);
...
{
	int rc = 0;
	//printk("**xip.c** %s()->pmfs_find_and_alloc_blocks().\n",__func__);
	...
	void **kmem, unsigned long *pfn)
{
	...
	sector_t block = 0;
	struct inode *inode = mapping->host;
	//printk("**xip.c** %s()->__pmfs_get_block().\n",__func__);
