		...
		struct inode *dir, struct dentry *dentry, struct inode *inode)
{
	...
	printk("**namei.c** %s()->pmfs_add_entry().\n",__func__);
...
restart:
	do {
		printk("**namei.c** %s() restart->pmfs_find_data_block.\n",__func__);
		blk_base =
				   ...
				   unsigned int flags)
{
	...
	printk("**namei.c** %s()->pmfs_inode_by_name(),ino=%lu,dentry->d_name->name:%s.\n",__func__,ino,dentry->d_name.name);
	if (ino) {
		//printk("**namei.c** %s()->pmfs_iget(), line=%d, ino= %lu,.\n",__func__,__LINE__,ino);
			...
			bool excl)
{
	...
	printk("**namei.c** %s()->pmfs_new_inode().\n",__func__);
	...
	printk("**namei.c** %s()->pmfs_add_nondir().\n",__func__);
...
{
	printk("**namei.c** pmfs_mkdir() start. dir:0x%x\n",dir->i_sb);
	struct inode *inode;
	...
	if (dir->i_nlink >= PMFS_LINK_MAX)
		goto out;
	//printk("**namei.c** pmfs_mkdir()->pmfs_new_transaction().\n");
	trans = pmfs_new_transaction(sb, MAX_INODE_LENTRIES * 2 +
			MAX_DIRENTRY_LENTRIES);
	if (IS_ERR(trans)) {
		err = PTR_ERR(trans);
	...
	printk("**namei.c** pmfs_mkdir()->pmfs_new_inode().\n");
	inode = pmfs_new_inode(trans, dir, S_IFDIR | mode, &dentry->d_name);
	printk("**namei.c** pmfs_mkdir(): inode: 0x%x, inode->i_ino:%lu, dentry->d_name:%s\n"
	,inode,inode->i_ino,dentry->d_name.name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode)) {
		pmfs_abort_transaction(sb, trans);
		goto out;
	 ...
	 */
	//trans=NULL,inode=inode,file_blocknr=0,num=1,zero=false
	//printk("**namei.c** pmfs_mkdir()->pmfs_alloc_blocks().\n");
	err = pmfs_alloc_blocks(NULL, inode, 0, 1, false);
	if (err)
		goto out_clear_inode;
	inode->i_size = sb->s_blocksize;
	//printk("**namei.c** pmfs_mkdir()->pmfs_get_block(sb,pmfs_find_data_block(inode,0)).\n");
	blk_base = pmfs_get_block(sb, pmfs_find_data_block(inode, 0));
	//printk("**namei.c** pmfs_mkdir(): inode->i_size: %lld,blk_base:%s\n",inode->i_size,blk_base);
	...
	strcpy(de->name, ".");
	printk("**namei.c** pmfs_mkdir(): de once: de->ino:%llu, de->de_len:%d, de->name:%s\n",de->ino,de->de_len,de->name);
	...
	de->name_len = 2;
	strcpy(de->name, "..");
	printk("**namei.c** pmfs_mkdir(): de twice: de->ino:%llu, de->de_len:%d,de->name:%s\n",de->ino,de->de_len,de->name);
	/*de->file_type =  S_IFDIR; */
	...
	printk("**namei.c** pmfs_mkdir() get pi from inode.\n");
	pi = pmfs_get_inode(sb, inode->i_ino);
	pmfs_memunlock_inode(sb, pi);
	pi->i_links_count = cpu_to_le16(inode->i_nlink);
	pi->i_size = cpu_to_le64(inode->i_size);
	pi->i_fsize = cpu_to_le64(inode->i_size);
	pi->i_fcount = 1;
	pi->i_awd = 1;//awd initialization.
	pmfs_memlock_inode(sb, pi);
	printk("**namei.c** pmfs_mkdir() pi->i_rsvd:%d,pi->root:%llu,pi->i_size:%llu,pi->i_links_count:%d,pi->i_blocks:%llu.\n",
	pi->i_rsvd,pi->root,pi->i_size,pi->i_links_count,pi->i_blocks);
	printk("**namei.c** %s()->pi->i_awd:%lu, pi->i_fcount=%lu\n",__func__,pi->i_awd,pi->i_fcount);
	
	printk("**namei.c** pmfs_mkdir() get pidir from dir.\n");
	pidir = pmfs_get_inode(sb, dir->i_ino);
	pmfs_inc_count(dir, pidir);
	d_instantiate(dentry, inode);//fill in inode information for a directory
	unlock_new_inode(inode);
	printk("**namei.c** pmfs_mkdir() pidir->i_rsvd:%d,pidir->root:%llu,pidir->i_size:%llu,pidir->i_links_count:%d,pidir->i_blocks:%llu.\n",
	pidir->i_rsvd,pidir->root,pidir->i_size,pidir->i_links_count,pidir->i_blocks);
	printk("**namei.c** %s()->pidir->i_awd:%lu\n",__func__,pidir->i_awd);

	pmfs_commit_transaction(sb, trans);
	printk("**namei.c** pmfs_mkdir() end.\n");
out:
	return err;
...
static int pmfs_empty_dir(struct inode *inode)
{
	...
	printk("**namei.c** %s()->pmfs_find_data_block once.\n",__func__);
	blk_base = pmfs_get_block(sb, pmfs_find_data_block(inode, 0));
	if (!blk_base) {
		pmfs_dbg("bad directory (dir #%lu)-no data block",
			  inode->i_ino);
	...
	while (offset < inode->i_size) {
		if (!blk_base || (void *)de >= (void *)(blk_base +
					sb->s_blocksize)) {
			err = 0;
			printk("**namei.c** %s()->pmfs_find_data_block twice.\n",__func__);
...
struct dentry *pmfs_get_parent(struct dentry *child)
{
	...
	if (ino){
		printk("**super.c** %s()->pmfs_iget(), line=%d, ino= %lu,.\n",__func__,__LINE__,ino);
