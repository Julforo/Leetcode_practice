...
{
	// printk("**inode.c** %s() start.\n",__func__);
	unsigned int data_bits = blk_type_to_shift[pi->i_blk_type];
	printk("**inode.c** %s()->pmfs_new_block():data_bits:%u,blocknr:0x%lx, sb->s_blocksize_bits:%u.\n"
	,__func__,data_bits,*blocknr,sb->s_blocksize_bits);
	int errval = pmfs_new_block(sb, blocknr, pi->i_blk_type, zero);
	printk("**inode.c** %s()pi->i_blocks before:%llu.\n",__func__,pi->i_blocks);
	if (!errval) {
	...
	printk("**inode.c** %s()pi->i_blocks after:%llu.\n",__func__,pi->i_blocks);
	// printk("**super.c** %s() end.\n",__func__);
	return errval;
}
...
u64 pmfs_find_data_block(struct inode *inode, unsigned long file_blocknr)
{
	...
	//printk("**inode.c** %s() convert the 4K blocks into the actual blocks the inode is using.\n",__func__);
	//printk("**inode.c** %s():blk_shift=0x%x,blk_offset:0x%lx,blocknr:0x%lx.\n",__func__,blk_shift,blk_offset,blocknr);
	
	if (blocknr >= (1UL << (pi->height * meta_bits)))
		return 0;
	//printk("**inode.c** %s()->__pmfs_find_data_block(),bp:0x%llx, return 0x%llx .\n",__func__,bp,bp + (blk_offset << sb->s_blocksize_bits));
	...
	if (bp == 0)
		return 0;
	// printk("**inode.c** %s() end, bp:0x%llx, return 0x%llx.\n",__func__,bp,bp + (blk_offset << sb->s_blocksize_bits));
	...
	bool zero)
{
	...
	first_index = first_blocknr >> node_bits;
	last_index = last_blocknr >> node_bits;
	//printk("**inode.c** %s():node:0x%llx,height=%u,node_bits:%u,first_index: %u, last_index:%u.\n",
	//	__func__,*node, height, node_bits, first_index, last_index);
	for (i = first_index; i <= last_index; i++) {
		if (height == 1) {
			if (node[i] == 0) {
				//printk("**inode.c** %s()->i=%d,pmfs_new_data_block().\n",__func__,i);
...
int __pmfs_alloc_blocks(pmfs_transaction_t *trans, struct super_block *sb,
	...
	bool zero)
{
	//printk("**inode.c** __pmfs_alloc_blocks start.\n");
	int errval;
	...
	//printk("**inode.c** %s():alloc_blocks height %d file_blocknr 0x%lx num 0x%x, "
	//	"first blocknr 0x%lx, last_blocknr 0x%lx.\n",
	//	__func__,pi->height, file_blocknr, num, first_blocknr, last_blocknr);
	height = pi->height;
	...
	if (!pi->root) {
		if (height == 0) {
			__le64 root;
			//printk("**inode.c** %s()->pmfs_new_data_block().\n",__func__);
			...
			//printk("**inode.c** %s() height=%d,pmfs_increase_btree_height().\n",__func__,height);
			...
			//printk("**inode.c** %s()->recursive_alloc_blocks() line=%d.\n",__func__,__LINE__);
	...
	//printk("**inode.c** __pmfs_alloc_blocks end.\n");
...
{
	printk("**inode.c** pmfs_init_inode_table() start.\n");
	struct pmfs_inode *pi = pmfs_get_inode_table(sb);
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	unsigned long num_blocks = 0, init_inode_table_size;
	int errval;
	printk("**inode.c** pmfs_init_inode_table(): pmfs_inode_table:0x%x.\n",pi);
	if (sbi->num_inodes == 0) {
	...
	num_blocks = (init_inode_table_size + pmfs_inode_blk_size(pi) - 1) >>
		pmfs_inode_blk_shift(pi);//(2MB+4k-1)>>12

	pi->i_size = cpu_to_le64(num_blocks << pmfs_inode_blk_shift(pi));
	/* pmfs_sync_inode(pi); */
	pmfs_memlock_inode(sb, pi);
	printk("**inode.c** %s()num_blocks:%lu.\n",__func__,num_blocks);
	sbi->s_inodes_count = num_blocks <<
		(pmfs_inode_blk_shift(pi) - PMFS_INODE_BITS);//512<<(12-7)
	printk("**inode.c** pmfs_init_inode_table(): init_inode_table_size:%lu, num_blocks:%lu, pi->i_size:%llu,sbi->s_inodes_count:%u.\n"
	,init_inode_table_size,num_blocks,pi->i_size,sbi->s_inodes_count);

	/* calculate num_blocks in terms of 4k blocksize */
	num_blocks = num_blocks << (pmfs_inode_blk_shift(pi) -
		sb->s_blocksize_bits);//512<<(12-12)... 
	//printk("**inode.c** pmfs_init_inode_table()(4K blocksize): num_blocks:%lu, sb->s_blocksize_bits:%d.\n",num_blocks,sb->s_blocksize_bits);//512
	//printk("**inode.c** pmfs_init_inode_table()->__pmfs_alloc_blocks() start.\n");
	errval = __pmfs_alloc_blocks(NULL, sb, pi, 0, num_blocks, true);
	//printk("**inode.c** pmfs_init_inode_table()->__pmfs_alloc_blocks() end.\n");
	...
	//printk("**inode.c** pmfs_init_inode_table(): sbi->s_free_inodes_count:%lu.\n",sbi->s_free_inodes_count);
	printk("**inode.c** pmfs_init_inode_table() end.\n");
	return 0;
}
...
static void pmfs_update_inode(struct inode *inode, struct pmfs_inode *pi)
{
	...
	//lfz aw
	pi->i_acfreq = cpu_to_le32(inode->i_count);
...
retry:
	...
	pi->height = 0;
	pi->i_dtime = 0;
	//file i_fcount=1;dir i_fcount=0
	pi->i_fcount = 0;//initialization
	pi->i_acfreq = 1;
	pi->i_awd = 0;//whether it is dir or regular file
	pmfs_memlock_inode(sb, pi);
...
/*lfz hot dir methods(init update and utilization)*/
/*
 * awd property update for a file
 * attain the parent dir of new file and then update its property
 * file_blocks can be zero, which depends on pmfs_xip_file_write_fast or xip_file_write
 * used scenarios: regular file -> create, read, write, append, delete, rename 
 */
static inline void pmfs_dir_update_awd(struct dentry *dentry, 
		struct pmfs_inode *pi, unsigned long file_blocks, unsigned int dir_flag)
{
	...
	{
		//create a file	
		case PMFS_FILE_CREATE:
			pidir->i_fcount += 1;
			pidir->i_fsize += file_blocks;
			break;
		//read a file
		case PMFS_FILE_READ:
			break;
		//write a file
		case PMFS_FILE_WRITE:
			pidir->i_fsize += file_blocks;
			break;
		//file append
		case PMFS_FILE_APPEND:
			pidir->i_fsize += file_blocks;
			break;
		//delete a file
		case PMFS_FILE_DELETE:
			pidir->i_fcount -= 1;
			pidir->i_fsize -= file_blocks;
			break;
		//rename a file
		case PMFS_FILE_RENAME:
			break;
		default:
			break;
	}
	pidir->i_acfreq += 1;
	pidir->i_awd = PMFS_DIR_AWD(pidir->i_fsize,pidir->i_fcount,pidir->i_acfreq);
	pmfs_memunlock_inode(sb,pi);
}
